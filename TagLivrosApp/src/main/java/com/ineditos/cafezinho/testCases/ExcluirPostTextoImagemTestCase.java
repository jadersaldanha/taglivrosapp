...
import com.ineditos.tasks.ExcluirPostTasks;
...
public class ExcluirPostTextoImagemTestCase {
	...
	WebDriver driver2;
	private ExcluirPostTasks excluir;

	@Before
	public void setUp() {
		Report.startTest("Excluir Post Texto e Imagem TestCase");
		...
		excluir = new ExcluirPostTasks(driver);
	}
	...
	public void testMain() throws Exception {
		...
		do {
			...
			if (driver.findElement(By.id("br.com.taglivros.tagineditos.debug:id/square_image_view")).isDisplayed()) {
				actions.press(PointOption.point(startX, startY))
						.waitAction(WaitOptions.waitOptions(Duration.ofMillis(500)))
						.moveTo(PointOption.point(endX, endY)).release().perform();

				if (driver.findElement(By.id("br.com.taglivros.tagineditos.debug:id/text_post"))
						.equals("Eu amo usar chinelos.")) {
					excluir.clicarMenuReticiencias();
					excluir.clicarDeletar();
					excluir.clicarSimDeletarPost();
				}
				//break;
			}
